...
#                 sent, state_word = self.wordRNN(torch.cuda.LongTensor(words).view(-1, self.batch_size), hid_state_word)
                sent, state_word = self.wordRNN(torch.LongTensor(words).view(-1, self.batch_size), hid_state_word) # for cpu only
                if sents is None:
                    sents = sent
                else:
    ...
    def init_hidden_hunk(self,use_gpu=True):
        if use_gpu:
    ...
    def init_hidden_sent(self,use_gpu=True):
        if use_gpu:
            return Variable(torch.zeros(2, self.batch_size, self.hidden_size)).cuda()
        else:
            return Variable(torch.zeros(2, self.batch_size, self.hidden_size))

    def init_hidden_word(self,use_gpu=True):
        if use_gpu:
            return Variable(torch.zeros(2, self.batch_size, self.hidden_size)).cuda()
        else:
            return Variable(torch.zeros(2, self.batch_size, self.hidden_size))
